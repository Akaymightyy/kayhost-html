...
async function inlineAssets(html, baseUrl, deadlineMs) {
  ...
  // BUG 3 FIX: Skip <script type="importmap"> tags — they contain JSON, not JS,
  // and inlining them as regular scripts corrupts the import map.
  html = await replaceAsync(html, /<script\b([^>]*)\bsrc\s*=\s*["']([^"']+)["']([^>]*)><\/script>/gi, async (scriptTag, beforeSrc, src, afterSrc) => {
    ...
    // BUG 3 FIX: Skip importmap scripts — leave them as-is (absolute URL)
    ...
    // Skip if the script tag has type=module with crossorigin — those need to stay external
    ...
    // BUG 3 FIX: Strip the type="importmap" attribute if present (shouldn't be, but defensive)
  ...
  html = await replaceAsync(html, /<img\b([^>]*)\bsrc\s*=\s*["']([^"']+)["']([^>]*)>/gi, async (imgTag, beforeSrc, src, afterSrc) => {
    ...
    if (timeRemaining() < 500) { timedOut = true; skippedCount++; console.log("[inline] IMG timeout-skip:", abs); return rewriteImgSrc(imgTag, src, abs, beforeSrc, afterSrc); }
    ...
    console.log("[inline] IMG OK:", abs, "(" + fetched.buf.length + " bytes)");
    return "<img" + beforeSrc + ' src="' + dataUri + '"' + afterSrc + ">";
  });

  // --- 1d. Inline CSS url() references inside <style> blocks AND inlined CSS ---
  // BUG 2 FIX: This now ACTUALLY INLINES fonts and images referenced in CSS url()
  // as base64 data URIs (if under 100KB), instead of just rewriting to absolute URLs.
  // This fixes the CORS errors when fonts/images are on a different domain.
  html = await replaceAsync(html, /<style\b[^>]*>([\s\S]*?)<\/style>/gi, async (styleTag, cssContent) => {
    if (timedOut) return styleTag;
    // Async-replace url() references inside the CSS
    const newCss = await replaceAsync(cssContent, /url\(\s*["']?([^"')]+)["']?\s*\)/gi, async (urlMatch, urlPath) => {
      ...
      if (timedOut) return 'url("' + abs + '")';  // fallback to absolute URL on timeout
      // Try to inline small assets (fonts, images) as base64 data URIs
      console.log("[inline] CSS url() fetch:", abs);
      const fetched = await fetchAssetBuffer(abs, Math.min(3000, timeRemaining()));
      if (!fetched.ok || !fetched.buf) {
      ...
      inlinedCount++;
      const dataUri = toDataUri(fetched.buf, fetched.contentType);
      console.log("[inline] CSS url() OK:", abs, "(" + fetched.buf.length + " bytes)");
      return 'url("' + dataUri + '")';
    });
    return "<style>" + newCss + "</style>";
  });

  // --- 1e. Rewrite @font-face src in inline <style> blocks ---
  // Already handled by 1d above (url() inlining covers @font-face src too).
