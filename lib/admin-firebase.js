...
function getAdmin() {
  ...
  if (!raw) {
    const err = new Error(
      "Server misconfigured: FIREBASE_SERVICE_ACCOUNT_KEY is not set. Generate one from Firebase Console → " +
      "Project Settings → Service Accounts → Generate new private key, then add the " +
      "full JSON as a Vercel environment variable."
    );
    err.status = 500; // distinct from a real 401 "not signed in" — this is a config problem, not the user's session
    throw err;
  }
  ...
  try {
    ...
    const err = new Error("Server misconfigured: FIREBASE_SERVICE_ACCOUNT_KEY is not valid JSON. Check the env var value.");
    err.status = 500;
    throw err;
  }
  ...
  if (!admin.apps.length) {
    try {
      ...
      const err = new Error("Server misconfigured: FIREBASE_SERVICE_ACCOUNT_KEY JSON looks valid but Firebase rejected it (" + e.message + "). Re-download the key from Firebase Console.");
      err.status = 500;
      throw err;
    }
  }
