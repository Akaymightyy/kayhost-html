...
async function callProvider(providerId, model, systemPrompt, userPrompt, options = {}) {
  ...
  try {
    ...
    if (!res.ok) {
      ...
      console.error("[ai-providers] " + providerId + " API error:", res.status, errorBody.slice(0, 300));
      // Map provider status to safe client-facing status (same as Ashna)
      let status = 502;
      let message = "AI provider error (" + providerId + ")";
      if (res.status === 429) {
        status = 429;
        message = "AI provider rate-limited, try again shortly";
      } else if (res.status === 401) {
        status = 503;
        message = providerId + " API key not configured or invalid. Check env var: " + (providerId === "opencode" ? "OPENCODE_API_KEY" : "AGENTROUTER_API_KEY");
      } else if (res.status === 404) {
        status = 502;
        message = providerId + " model not found. Check model ID.";
      } else if (res.status >= 500) {
        status = 502;
        message = providerId + " server error";
      }
