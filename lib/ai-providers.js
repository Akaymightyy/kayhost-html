...
async function callProvider(providerId, model, systemPrompt, userPrompt, options = {}) {
  ...
  try {
    ...
    if (!res.ok) {
      // Log the REAL provider response server-side (Vercel function logs) so the
      // site owner can actually diagnose this — bad key, no credit, wrong model
      // ID, etc. — without exposing the raw provider response to the client.
      let providerDetail = "";
      ...
      console.error(`[ai-providers:${providerId}] HTTP ${res.status} calling ${baseUrl} with model "${model}": ${providerDetail}`);

      // Map provider status to a safe, but now status-code-aware, client-facing message.
      let status = 502;
      ...
      if (res.status === 429) {
        ...
        status = 502;
        message = `AI provider rejected the request (HTTP ${res.status}) — check ${providerId === "opencode" ? "OPENCODE_API_KEY" : "AGENTROUTER_API_KEY"} on Vercel.`;
      } else if (res.status === 404) {
        status = 502;
        message = `AI provider error (HTTP 404) — model "${model}" may not exist on this provider.`;
      }
