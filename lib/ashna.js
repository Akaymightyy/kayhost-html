...
async function fetchAshnaModels() {
  ...
  try {
    ...
    const rawList = Array.isArray(data?.data) ? data.data
                 ...
                 : [];
    const liveIds = new Set(rawList.map(m => m && m.id).filter(Boolean));

    // Keep IDs in BOTH the allowlist AND the live response
    ...
    const defaultModel =
      ASHNA_DEFAULT_MODEL && liveIds.has(ASHNA_DEFAULT_MODEL) && allowlist.includes(ASHNA_DEFAULT_MODEL)
        ? ASHNA_DEFAULT_MODEL
...
// intersection of allowlist + live /models response). Returns boolean.
// Call this BEFORE calling Ashna — a model not in the catalog would fail
// server-side anyway, but checking here lets us return 400 instead of 502.
...
async function callAshnaChat(model, systemPrompt, userPrompt, options = {}) {
  ...
  try {
    ...
    if (!res.ok) {
      // Map Ashna status to safe client-facing status
      let status = 502;
      ...
      if (res.status === 429) {
        ...
        // 400/401/403/404 — don't expose the reason
        status = 502;
        message = "AI provider error";
      }
