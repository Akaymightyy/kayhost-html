...
async function callAshnaChat(model, systemPrompt, userPrompt, options = {}) {
  ...
  try {
    ...
    // Parse response with JSON-first, HTML-fallback (same fix as lib/ai-providers.js).
    // A successful HTML edit will legitimately contain "<html" / "<!doctype html>"
    // in the message content — that's the whole point. So we try JSON.parse FIRST,
    // and only check for an HTML error page if parsing genuinely fails.
    ...
    try {
      ...
      if (trimmed.startsWith("<!doctype html") || trimmed.startsWith("<html")) {
        const err = new Error("AshnaAI returned an HTML error page instead of JSON (likely a gateway or firewall block).");
        err.status = 502;
        throw err;
      }
      const err = new Error("AshnaAI returned invalid response.");
      err.status = 502;
      throw err;
    }
